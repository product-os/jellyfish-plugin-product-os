...
const handler = async (session, context, card, request) => {
  ...
  if (matcher) {
    ...
    const safeWorkerQuery = skhema.merge([matcher, {
      type: 'object',
      required: ['active'],
      properties: {
        active: {
    ...
    const workers = await context.query(context.privilegedSession, safeWorkerQuery)

    // Sort the agents by the best match
    const [bestMatchedWorker] = _.reverse(_.sortBy(workers, (item) => {
    ...
    if (bestMatchedWorker) {
      ...
      logger.info(request.context, 'Could not find a matching worker for task', {
        id: card.id,
        slug: card.slug,
        type: card.type
      })
